...
import { ArrowLeft, Skull, AlertTriangle, Clock, MapPin, User } from 'lucide-react';
...
export function Alerts() {
  const getLevelColor = (level: string) => {
    switch (level) {
      case 'critical': return 'border-red-500 bg-red-600/20 text-red-400';
      case 'high': return 'border-orange-500 bg-orange-600/20 text-orange-400';
      case 'medium': return 'border-yellow-500 bg-yellow-600/20 text-yellow-400';
      case 'low': return 'border-green-500 bg-green-600/20 text-green-400';
      default: return 'border-gray-500 bg-gray-600/20 text-gray-400';
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'cleared': return 'text-cyan-400';
      case 'neutralized': return 'text-blue-400';
      ...
      <div className="relative z-10 container mx-auto px-6 py-8 max-w-6xl">
        <header className="mb-8 border-b border-cyan-500/30 pb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-cyan-400 uppercase tracking-wider">Threat Alerts</h1>
                <p className="text-gray-500 text-sm">Security Event Monitoring & Response</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-black/40 border border-red-500/30 rounded-lg p-4 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-400 uppercase tracking-wider text-xs mb-1">Critical</p>
          ...
          <div className="bg-black/40 border border-orange-500/30 rounded-lg p-4 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-orange-400 uppercase tracking-wider text-xs mb-1">High</p>
          ...
          <div className="bg-black/40 border border-yellow-500/30 rounded-lg p-4 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-yellow-400 uppercase tracking-wider text-xs mb-1">Medium</p>
          ...
          <div className="bg-black/40 border border-green-500/30 rounded-lg p-4 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-green-400 uppercase tracking-wider text-xs mb-1">Low</p>
        ...
        <div className="bg-black/40 border border-cyan-500/30 rounded-lg p-6 backdrop-blur-sm">
          <h2 className="text-cyan-400 uppercase tracking-wider mb-6">Recent Alerts</h2>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="uppercase tracking-wider mb-1">{alert.type}</h3>
                      <p className="text-xs text-gray-400 font-mono">{alert.timestamp}</p>
                    </div>
                  </div>
                  <span className={`text-xs uppercase tracking-wider ${getStatusColor(alert.status)}`}>
                    {alert.status}
                  </span>
                </div>

                <p className="text-gray-300 mb-3 text-sm">{alert.description}</p>
